...
def menuAcceso(request):
    ...
    comando = 'SELECT ltrim(codreg_sede), nom_sede FROM public."Administracion_imhotep_sedesreportes" '
    cur.execute(comando)
    print(comando)
...
def validaAcceso(request):
    ...
    return render(request, "Reportes/fachada.html", context)
...
class Reporte1PdfView(TemplateView):
    ...
    def myFirstPage(self, canvas, doc):
        ...
        print(fecha)
        print(format)
        canvas.drawImage("C:\\FONDOCM.jpg", 40, 715, width=50,
                     height=50)
        canvas.drawString(250, 750, "CLINICA MEDICAL")
        canvas.drawString(250, 735, 'NIT: 99999999999-99')
    ...
    def get_context_data(self, **kwargs):
        ...
        print("ESte es el contexto numeroReporte = ", context['numreporte'] )
        ...
        for numreporte,  reporte, cuerpo_sql, descripcion, encabezados, excel, pdf, csv  in cur.fetchall():
            reporteSeleccionado.append(
                {'numreporte': numreporte,  'reporte': reporte, 'cuerpo_sql': cuerpo_sql,
                 'descripcion': descripcion, 'encabezados':encabezados, 'excel':excel, 'pdf':pdf, 'csv':csv})
        ...
        comando = 'select  reportes.id numreporte, usuarios.cod_usuario usuario, reportes.nom_reporte reporte,reportes.cuerpo_sql, reportes.descripcion descripcion , reportes.encabezados encabezados from public."Administracion_mae_repusuarios" as usuarios,public."Administracion_mae_reportes" as reportes , public."Administracion_imhotep_sedesreportes" sedes  where usuarios.cod_Usuario = ' + "'" + username + "'" + ' and  usuarios.mae_reportes_id = reportes.id  and usuarios.cod_sede_id = sedes.id  and sedes.codreg_sede = ltrim(' + "'" + str(sedeSeleccionada) + "')"
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (tipoArchivo == "excel"):
           ...
           #     ws.write(idx, 1, styles[k], style)
            ...
            #row_num = row_num + 1
            #Info0.write(row_num, 0, 'NIT: 99999999999-99', font_style)
            #row_num = row_num + 1
...
def titulosCab(RecibeInfo):
        ...
        row_num = row_num + 1
        RecibeInfo.write(row_num, 0, 'NIT: 99999999999-99', font_style)
        row_num = row_num + 1
